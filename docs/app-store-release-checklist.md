...
- Confirm Google OAuth is already enabled in `supabase/config.toml` and was pushed with the current credentials.
...
- `store/storekit-setup.md` StoreKit section is complete.
...
- Cloudflare DNS for `nitime.app` still needs the owner steps in

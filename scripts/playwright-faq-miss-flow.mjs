...
const inboundMessage =
  process.env.E2E_INBOUND_TEXT ?? 'Can you do a custom 3-hour package next month?';
const receiptSnippet =
  process.env.E2E_RECEIPT_SNIPPET ?? "Thanks — your message has been received";

async function main() {
  const { runId, consoleMessages } = createRunContext('faq-miss');
  ...
  try {
    ...
    await widgetPage.getByText(new RegExp(receiptSnippet, 'i')).first().waitFor({
      state: 'visible',
      timeout: 15000,
    });
    await screenshot(widgetPage, runId, '11-widget-receipt');

    const pendingDraft = await waitForPendingDraftForInboundMessage(email, inboundMessage);
    if (pendingDraft.source === 'faq') {
      await fail(widgetPage, runId, 'FAQ miss incorrectly routed through FAQ source.');
    }

    const leakedDraft = widgetPage.getByText(pendingDraft.text, { exact: true }).first();
    if (await leakedDraft.isVisible().catch(() => false)) {
      await fail(widgetPage, runId, 'Pending draft leaked to the visitor before approval.');
    }
    ...
    await page.getByText(pendingDraft.text, { exact: true }).waitFor({ state: 'visible', timeout: 15000 });
    ...
    console.log('Playwright FAQ miss validation passed.');
